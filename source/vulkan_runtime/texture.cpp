...
namespace vulkan_runtime {
    ...
    vulkan_texture runtime::create_texture(const std::vector<unsigned char> &texture, const int width, const int height, const VkFormat format) {
        ...
        VMA::image_info image_info = {};
        image_info.format = format;
        image_info.height = height;
        image_info.width = width;
        image_info.array_layers = 1;
        image_info.mip_levels = 1;
        const auto handler = this->core.vma.create_image_with_data(std::span(texture), image_info, VMA::image_type::texture_2d); // NOLINT(*-misplaced-const)
        const auto& [image, allocation] = core.vma.get_image(handler); // NOLINT(*-misplaced-const)
        ...
        VkDeviceMemory image_memory = this->core.vma.get_device_memory(allocation);
